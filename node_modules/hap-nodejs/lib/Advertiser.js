...
Advertiser.prototype.startAdvertising = function(port) {
  ...
  this._advertisement = mdns.publish({ name: this.accessoryInfo.displayName, type: 'hap', port: port, txt: txtRecord, host: OS.hostname().indexOf('.local') > -1 ? OS.hostname() : OS.hostname() + '.local' });
...
Advertiser.prototype.updateAdvertisement = function() {
  if (this._advertisement) {
    ...
    this._advertisement.txt = txtRecord;
  }
}
