...
function updateStatus(device,variable,status,callback)
{
  ...
  //console.log(device.name + ': getAvailable ' + device.Available, 'info');
...
//  console.log('device: '+ JSON.stringify(device));
  ...
  if ('onoff' in device.capabilities)
  {
    ...
    {
      var temp = HAS.predefined.Saturation(characteristicNum++, Math.floor(map(0, 1, 50, 400, device.state.light_temperature || 400)), (value, callback) =>
      {
  ...
  device.on('$state', state =>
  {
    ...
    if (!'light_saturation' in device.capabilities && !'light_hue' in device.capabilities && 'light_temperature' in device.capabilities)
    {
      ...
      if (variable != null)
      {
        variable = Math.floor(map(0, 1, 50, 400, variable));
