...
function updateStatus(device,value,status,callback)
{
  let variable = value;
  if (device.state[status] != variable)
  {
...
function updateStatusNoCallback(device,value,status)
{
  let variable = value;
  if (device.state[status] != variable)
  {
...
function updateStatusOnHK(variable, deviceHK, status, device)
{
  if (variable != null && deviceHK.value != variable)
  {
...
//  console.log('device: '+ JSON.stringify(device));
  ...
  if ('onoff' in device.capabilities)
  {
    ...
    if ('light_hue' in device.capabilities)
    {
      ...
      {
        updateStatus (device, value / 360, "light_hue", callback);
      });
    ...
    if (!'light_saturation' in device.capabilities && !'light_hue' in device.capabilities && 'light_temperature' in device.capabilities)
    {
      ...
      {
        updateStatus (device, map(50, 400, 0, 1, value), "light_temperature", callback);
      });
  ...
  if ('speaker_playing' in device.capabilities)
  {
    ...
    newDevice.addServices(HAS.predefined.Switch(serviceNum++, [onSpeakerPlaying]));
  }
  ...
  if ('speaker_next' in device.capabilities)
  {
    ...
    newDevice.addServices(HAS.predefined.Switch(serviceNum++, [onSpeakerNext]));
    //capabilities = [];
  }
  ...
  if ('speaker_prev' in device.capabilities)
  {
    ...
    newDevice.addServices(HAS.predefined.Switch(serviceNum++, [onSpeakerPrev]));
  }
  ...
  device.on('$state', state =>
  {
    ...
    if ('volume_mute' in device.capabilities)
    {
      updateStatusOnHK(state.volume_mute, mute, "volume_mute", device);
    }
    ...
    if (!'light_saturation' in device.capabilities && !'light_hue' in device.capabilities && 'light_temperature' in device.capabilities)
    {
      ...
      if (variable != null)
      {
        variable = map(0, 1, 50, 400, variable);
    ...
    if ('alarm_motion' in device.capabilities)
    {
      updateStatusOnHK(state.alarm_motion, motion, "alarm_motion", device);
    }

    if ('measure_humidity' in device.capabilities)
    {
      updateStatusOnHK(state.measure_humidity, currentRelativeHumidity, "measure_humidity", device);
    }
    ...
    if ('measure_temperature' in device.capabilities)
    {
      updateStatusOnHK(state.measure_temperature, temperature, "measure_temperature", device);
    }
    ...
    if ('speaker_playing' in device.capabilities)
    {
      let variable = device.state.speaker_playing || false;
