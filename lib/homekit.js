...
function createSocket(device) {
  console.log('SOCKET: Found capability ONOFF');
  ...
  if ('onoff' in device.capabilities) {
    // Switch the capability on user input
    var on = HAS.predefined.On(8, device.state.onoff, (value, callback) => {
      // Calling the api
      device.setCapabilityValue("onoff", value)
      callback(HAS.statusCodes.OK);
    });

    var inuse = HAS.predefined.OutletInUse(7, true, undefined);
    // Push to array
    capabilities.push(on);
