...
const isThrottled = () => {
  const now = Date.now();
  if (now - throttleTimer < 500) return true;
  throttleTimer = now;
  return false;
};
...
const MAP = {
  ...
  button:        W('button',        Characteristic.ProgrammableSwitchEvent, {
    get: v => 0,    // convert to "Single Press"
    set: v => true,
  }),
  dim:           W('dim', Characteristic.Brightness, MapPercentage),
  homealarm_state: [
    W('homealarm_state', Characteristic.SecuritySystemCurrentState, {
      get: v => ( { armed : 1, disarmed : 3, partially_armed : 0 }[v] ),
      set: v => [ 'partially_armed', 'armed', 'partially_armed', 'disarmed', 'armed' ][v],
    }),
  ...
  light_temperature: W('light_temperature', Characteristic.ColorTemperature, {
    get: v => mapColorTemperature(v, 0, 1, 140, 500),
    set: v => mapColorTemperature(v, 140, 500, 0, 1),
  }),
  locked:            [
  ...
  target_temperature: [
    ...
    //  already set in `measure_temperature`
  ],
  thermostat_mode:     [
    W('thermostat_mode', Characteristic.CurrentHeatingCoolingState, {
      get: v => ( { off : 0, heat : 1, cool : 2 }[v] || 3 ), // 3 === 'auto'
      set: v => [ 'off', 'heat', 'cool' ][v]         || 'auto',
    }),
  ...
  windowcoverings_state: W('windowcoverings_state', Characteristic.PositionState, {
    get: v => ( { up : 1 , idle : 2, down : 0 }[v] ),
    set: v => [ 'down', 'up', 'idle' ][v],
  }),
};
