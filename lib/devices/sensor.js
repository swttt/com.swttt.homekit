...
module.exports = function(device, api, capabilities) {
  ...
  if ('measure_luminance' in capabilities) {
    homekitAccessory
      ...
      .on('get', function(callback) {
        callback(null, capValue('measure_luminance') ?? this.value);
      });
  }
  ...
  if ('measure_temperature' in capabilities) {
    homekitAccessory
      ...
      .on('get', function(callback) {
        callback(null, capValue('measure_temperature') ?? this.value);
      });
  }
  ...
  if ('measure_humidity' in capabilities) {
    homekitAccessory
      ...
      .on('get', function(callback) {
        callback(null, capValue('measure_humidity') ?? this.value);
      });
  }
    ...
    var coService = homekitAccessory
      ...
      if ('measure_co' in capabilities) {
        coService.getCharacteristic(Characteristic.CarbonMonoxideLevel)
          .on('get', function(callback) {
            callback(null, capValue('measure_co') || this.value);
          });
      }
    ...
    var co2service = homekitAccessory
      ...
      if ('measure_co2' in capabilities) {
        co2service.getCharacteristic(Characteristic.CarbonDioxideLevel)
          .on('get', function(callback) {
            callback(null, capValue('measure_co2') || this.value);
          });
      }
    ...
    switch(capability) {
      ...
      case 'measure_luminance': // Light Sensor
        homekitAccessory
          ...
          .getCharacteristic(Characteristic.CurrentAmbientLightLevel)
          .updateValue(capValue('measure_luminance') ?? this.value);
        break;
      case 'measure_temperature': // Temperature Sensor
        homekitAccessory
          .getService(Service.TemperatureSensor)
          .getCharacteristic(Characteristic.CurrentTemperature)
          .updateValue(capValue('measure_temperature') ?? this.value);
        break;
      case 'measure_humidity': // Humidity Sensor
        homekitAccessory
          .getService(Service.HumiditySensor)
          .getCharacteristic(Characteristic.CurrentRelativeHumidity)
          .updateValue(capValue('measure_humidity') ?? this.value);
        break;
