...
module.exports = function(device, api) {
  ...
  device.on('$capabilitiesObj', debounce(capabilitiesObj => {
    let stateKeys  = Object.keys(capabilitiesObj);
    const capValue = key => capabilitiesObj[stateKeys.filter(k => k.startsWith(key))[0]].value;

    console.log('Realtime update from:', device.name, ', state =', device.capabilitiesObj);
