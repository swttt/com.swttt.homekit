...
module.exports = function(device, api, capabilities) {
  ...
  if ('onoff' in capabilities) {
    const service = homekitAccessory.addService('dim' in capabilities ? Service.Lightbulb : Service.Outlet, device.name);
  ...
  device.on('$state', debounce(state => {
    ...
    // Outlet/light
    if ('onoff' in capabilities) {
      const service = homekitAccessory.getService('dim' in capabilities ? Service.Lightbulb : Service.Outlet);
      service
        .getCharacteristic(Characteristic.On)
