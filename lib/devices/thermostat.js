...
module.exports = function(device, api, capabilities) {
  ...
  device.on('$update', id => {
    ...
    homekitAccessory
      .getService(Service.Thermostat)
      .getCharacteristic(Characteristic.TargetTemperature)
      .updateValue(device.capabilitiesObj.target_temperature.value);

    homekitAccessory
      .getService(Service.Thermostat)
      .getCharacteristic(Characteristic.CurrentTemperature)
      .updateValue(device.capabilitiesObj.measure_temperature.value);

    if ('measure_humidity' in capabilities) {
      homekitAccessory
          .getService(Service.Thermostat)
          .getCharacteristic(Characteristic.CurrentRelativeHumidity)
          .updateValue(device.capabilitiesObj.measure_humidity.value);
    }
    ...
    if ('thermostat_mode' in capabilities) {
      homekitAccessory
        .getService(Service.Thermostat)
        .getCharacteristic(Characteristic.CurrentHeatingCoolingState)
        .updateValue(state2value(device.capabilitiesObj.thermostat_mode.value));
      }
