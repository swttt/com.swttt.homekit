...
module.exports = function(device, api) {
  ...
  homekitAccessory
    ...
    .on('set', function(value, callback) {
	  let state = value2state(value);
	  if (state == 'idle')
	  {
		console.log('Skipping the blinds idle state');
	  }
	  else
	  {
		console.log('Setting blinds to: ' + state);
		device.setCapabilityValue("windowcoverings_state", state);
	  }
      callback();
    })
    .on('get', function(callback) {
