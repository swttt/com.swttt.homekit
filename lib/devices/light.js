...
module.exports = function(device, api) {
  ...
  device.on('$capabilitiesObj', debounce(capabilitiesObj => {
    ...
    homekitAccessory
      .getService(Service.Lightbulb)
      .getCharacteristic(Characteristic.On)
      .updateValue(capabilitiesObj['onoff'].value);
    ...
    if ('light_saturation' in capabilities) {
      homekitAccessory
        ...
        .updateValue(capabilitiesObj['light_saturation'].value * 100);
    }
	if ('light_temperature' in capabilities) {
      homekitAccessory
        .getService(Service.Lightbulb)
        .getCharacteristic(Characteristic.ColorTemperature)
        .updateValue(map(0, 1, 140, 500, capabilitiesObj['light_temperature'].value));
    }
    if ('light_hue' in capabilities) {
      homekitAccessory
        .getService(Service.Lightbulb)
        .getCharacteristic(Characteristic.Hue)
        .updateValue(capabilitiesObj['light_hue'].value * 360);
    }
