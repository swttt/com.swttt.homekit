...
module.exports = function(device, api) {
  ...
  device.on('$capabilitiesObj', debounce(capabilitiesObj => {
	...
	//support for Heimdall app alarm capability
	if('alarm_heimdall' in device.capabilities && capabilitiesObj['alarm_heimdall'].value === true){
      homekitAccessory
        .getService(Service.SecuritySystem)
        .getCharacteristic(Characteristic.SecuritySystemCurrentState)
        .updateValue(4);
	...
	{		
      homekitAccessory
	    .getService(Service.SecuritySystem)
	    .getCharacteristic(Characteristic.SecuritySystemTargetState)
	    .updateValue(state2value(capabilitiesObj['homealarm_state'].value, targetStateValue));
    ...
    if('alarm_tamper' in device.capabilities){
      homekitAccessory
        .getService(Service.SecuritySystem)
        .getCharacteristic(Characteristic.StatusTampered)
        .updateValue(~~capabilitiesObj['alarm_tamper'].value);
    }
