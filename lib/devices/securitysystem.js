...
module.exports = function(device, api, capabilities) {
  // Init device
  let homekitAccessory = new Accessory(device.name, device.id);
  ...
  homekitAccessory
    ...
    .on('get', function(callback) {
      ...
      if ('alarm_heimdall' in device.capabilitiesObj && device.capabilitiesObj.alarm_heimdall.value === true) {
        ...
        return callback(null, value ? state2value(value, targetStateValue) : this.value);
      } else if ('onoff' in device.capabilities) { // Satel Integra
        const value = getCapabilityValue(device, 'onoff');
        return callback(null, value ? state2value(value ? 'armed' : 'disarmed', targetStateValue) : this.value);
      }
  ...
  homekitAccessory
    ...
    .on('set', function(value, callback) {
      ...
      } else if ('onoff' in device.capabilities) {
        device.setCapabilityValue('onoff', value ? 'armed' : 'disarmed');
      }
      return callback();
    })
    .on('get', function(callback) {
