...
const { getCapabilityValue, setupAccessoryInformations } = require('./utils');
...
module.exports = function(device, api, capabilities) {
  ...
  for (let i in device.capabilities) {
    if (device.capabilities[i] && [ 'alarm_heimdall', 'homealarm_state', 'alarm_tamper', 'onoff' ].includes(device.capabilities[i].split('.')[0])) {
